# Our Team

**Last updated:** 2026.09.01
**Source:** HPC-MKTG knowledge base (2026-08-26), Evan's corrections (2026-09-01).

---

## Inside HPC

| Person | Role | What you need to know |
|---|---|---|
| **Jay Meral** | Founder / owner | The audience for top-level CAC/ROAS reporting. Owns the dealer/wholesale lane. Keep his artifacts simple and honest. |
| **Robert** | **Co-owner**, product designer | Creator of the **Boil Boss** line — HPC absorbed Boil Boss and now manufactures it in-house. Designs new products. **Reads the marketing reports closely and numerately.** |
| **Evan Meral** | **Lead marketer** — the operator of this brain | Sole day-to-day marketing operator. Runs Meta ads himself, does all social posting manually, handles design by default. |
| **Stephen** | Manager of the **commercial product section** + **commercial shipping/receiving lead** | Talks commercial buyers through what they're getting and how it ships, so freight is never a checkout surprise on LTL orders. Also runs commercial shipping and receiving. |
| **Todd Tunstall** | Original creator of the Tunnel Tube technology | Historical/story credit. |

**About twelve people total** do everything — welding tunnel tubes by hand, packing freight, answering the phone. *"A small team of 12 that do everything. No big corporate entity here."* That line is a genuine trust asset — use it.

---

## Vendors and partners

| Who | Scope | Cost |
|---|---|---|
| **Biljana** | Contract — Klaviyo / email | Rate unknown |
| **IntentWave** | Meta ads advisory. Also runs an **identity pixel** on the site and syncs audiences into Meta. | **Free** |
| **Coalition** | **Google Ads** (revamp began ~mid-Aug 2026) + **HPC SEO for 2+ years** | **$4,800/month** (management only — ad spend separate) |
| **IntentWave** *(operates as **Persistent.id**)* | Meta advisory, identity pixel + Meta audience syncs, and the **site cookie-consent tool** | Free |
| **Garrett Frazier / Frazier Media** | Videographer, engaged ~19 Aug 2026 | **$500/video, or $2,500 for 8** ($312.50 each) |

**On Coalition's fee:** $4,800/mo is ~27.5% of the ~$17.4K/mo Google spend — above the usual 10–20% for pure ad management, **but it also covers SEO, and organic search is 50% of first-touch discovery.** On that basis it looks reasonable. Reconcile results against Shopify periodically — hygiene, not distrust.

**On Garrett:** recommend the 8-video bundle over per-video whenever 5+ videos are planned.


### Who's who at the vendors

**Coalition Technologies** — work runs through a shared **Basecamp** project, `| High Performance Cookers | // Joint // SEO & PPC`:
- **Caroline Giercyk** — Sr. Digital Producer. Main day-to-day contact, runs alignment meetings.
- **Connor Levy** — PPC.
- **Andrea Montgomery** — content/copy batches and PR opportunities.
- **Charles Robertson**, **Guillermo Aguirre**, **Justin LePatourel** (built the new site).

**IntentWave / Persistent.id** — **Dalton MacKinnon** (books the Meta reviews), plus Mike, Tommy and Teresa. They run recurring Meta review calls and an email sync.

**Biljana** — reachable at her personal Gmail. Still fully her lane; this is contact info only.

### Email addresses that matter

- **Evan: `evan@highperformancecookers.com`** — the canonical address, and the one he types out in full. `evan@hpcookers.com` appears to route to the same mailbox (Basecamp uses it), but **always write the full one**.
- **Robert: `robert@lapeyre.dev`** — the address to send to. He also appears as `robert@theboilboss.com`; **ignore that one.**
- Jay: `jay@highperformancecookers.com` · Stephen: `stephen@highperformancecookers.com`

---

## ⛔ LANE DISCIPLINE — who owns what

This is the most important operating rule in this file. Two parties operating the same account creates conflicting changes and destroys accountability.

| Lane | Owner | What this brain does |
|---|---|---|
| **Meta Ads** | **Evan** (IntentWave advises) | Recommend concrete changes freely. Still always Evan's click. |
| **Google Ads + SEO** | **Coalition** | **Monitor and report only.** Never propose that Evan make direct changes in Google Ads. Package suggestions as a document Evan can forward — framed as data Coalition may not have (break-even ROAS, per-SKU margin), never as instructions. |
| **Email / SMS / Klaviyo** | **Biljana — FULLY HERS** | ⛔ **Stay out entirely.** Do not propose email work, do not ask for Klaviyo access, do not analyse email performance. Evan has said no twice. `playbooks/email-sms.md` is reference only. |
| **Video** | **Garrett / Frazier Media** | Brief, don't shoot. |
| **Dealer / wholesale** | **Jay** | Route inbound to him. No marketing investment. |
| **Commercial sales + freight** | **Stephen** | He qualifies residential vs. business early so freight isn't a surprise. No fix needed there. |

---

## ⚠️ Verify every vendor's numbers

**Never take a marketing vendor's self-reported results at face value.** Reconcile every claim against Shopify revenue before repeating it to Evan, Jay, or Robert.

**Why:** HPC hired **BM Digital (now Advantage HQ)** to run influencer sourcing and Meta ads. They claimed they generated **more than three-quarters of HPC's annual revenue** while actually shipping free product to creators who never posted. It cost real money and produced no attributable sales. The claim was checkable against total revenue and nobody checked it.

**How to apply:** Judge partners on **incremental revenue**, never on activity metrics (creators contacted, product shipped, posts promised). Never ship seeded product without a signed deliverable and a deadline. Frame this as hygiene, not distrust.

**📅 2026-07-20 is the analytical dividing line.** That is the exact day BM Digital was fired (Evan, confirmed 2026-09-01 — use this date over any other). BM Digital ran both Meta and Google until then. Evan took Meta in-house; Coalition took Google. **Any data spanning that date is two different operators and must not be read as one trend.**

---

## How to work with Evan

- **He is the hub, and the bottleneck.** Nearly every spoke routes through him. He's full-time on marketing (~40 hrs/wk), so **the constraint is not hours, it's structure.** His words: *"everything is kind of all over the place."*
- He posts to every channel manually through native apps, one at a time, and has never used a scheduling tool.
- **The highest-value help is imposing a system and removing manual repetition** — batching, scheduling, repurposing — not finding him time.
- There is no dedicated designer, editor, or social manager.
- He wants **peer-level, opinionated, direct communication. No corporate hedging.**

## How to write for Robert and Jay

Both read the reports. Write for a **numerate co-owner**:
- Name the source of every figure.
- Separate **contribution margin** from **net profit**.
- Never present platform-reported revenue as actual revenue.
- Robert asks for **per-product economics from landed BOM**, not blended estimates.
- Expect scrutiny, and welcome it — on 2026-08-28 he corrected the CAC model and was right.

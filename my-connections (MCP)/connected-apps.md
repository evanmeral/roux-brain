...
| **Meta Ads Manager** | ❌ Still no connector | Browser (read-only) via Claude in Chrome, or CSV export into `my-work (outputs)/internal/reports/raw/` |
...
Still **no direct connector** for the three systems that matter most:

| System | Status | Workaround |
|---|---|---|
| **Shopify admin** | Source of truth for orders/revenue/product mix — **no connector** | Browser (read-only) via Claude in Chrome, or CSV export into `my-work (outputs)/internal/reports/raw/` |
| **Meta Ads Manager** | Source of truth for Meta — **no connector** | Browser (read-only), or CSV export |
| **Google Ads** | Source of truth for Google — **no connector** | Browser (read-only), or Coalition's reporting |

**Consequence:** every profitability recommendation is unverifiable without one of these. **Never invent or estimate a performance number** — say "I don't have that number," then pull it read-only from the browser or ask Evan for a CSV.

**Read-only always.** Ask before changing anything in Shopify, Meta, or Google.

...
## 2026-09-24 — Sept 28 week: shoot footage swapped in
- **Friday Oct 2 reel waits for Garrett's cut, not Evan's phone clips** (Evan: "we will be having a professional video coming from garrett and probably better raw footage"). A phone-clip cut was built, then set aside in `fallback/`. Still dropped if Garrett's cut misses Thu Oct 1 noon. The pot is the 30 QT (Evan).
- **Shop Floor Tuesday uses the shoot-day shop-front photo** (Evan: "we can use that for the krewe post"). The booth photo is kept in `fallback/`.
- **Blue-flame shot:** Evan's Sept 23 attempt may not be clear; he reshoots it next time the pots are in use (Evan).
- **Friday Oct 2 reel dropped entirely**; the week runs five (Evan, 2026-09-24: "we can drop this post entirely since we will probably do this one closer to thanksgiving"). Turkey reel → November, from Garrett's cut.
- **Approved in ROUX OS 2026-09-24:** 3-carousel (How-To Wed), 4-feed (Word of Mouth Thu), 6-story (Game Day Sat). Approval only; nothing scheduled yet.
- **Monday 4mm v2:** photo band taller so the four pots show whole (Evan: the black backdrop covered the pots). **Tuesday:** Evan says the image "is lacking something"; two options built (A: 4-photo raw carousel of the shoot day; B: shop-front photo with the shield, kicker and "Camera crew in. Krewe on."). Waiting on his pick.
- **Content notes, round 2 (Evan, ROUX OS 2026-09-24):** Monday headline → "Thin pots warp. Ours don't." (the 4mm is already up top). Tuesday carousel gets short copy and the shield on every frame ("it looks lazy that we're posting a standard image off an iPhone"); rule 2 (raw stays raw) set aside for this post. Tuesday story: no blurred band, full-bleed, short copy. Option B folded into the cover frame.
- **Content notes, round 3 (Evan, ROUX OS 2026-09-24):** Monday photo gets the missing single jet tube under the far-left pot (new asset `powered-pot-lineup-patio-jet-tube-fixed-2026-09-24.png`; use it over the original). Tuesday cover → "Shoot day at the shop.", last frame → "Best part of shoot day.", story → "Shoot day at the shop." / VIDEOS COMING SOON.

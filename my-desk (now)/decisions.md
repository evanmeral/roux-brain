...
**No cap on skills; overlapping starter skills get retired; Approvals cards are short, evidence folded** · Evan, 2026-09-25 ("dont limit yourself to 30 skills… if things start overlapping… go ahead and clean up/remove old ones"; Approvals "have way too much info… make the verbiage short"). The 30-skill cap in `/improve` was ROUX's own number, never Evan's; removed. `/improve` now retires an old starter skill that fires on the same request as a newer one without waiting for Approvals (archived to `archive/retired-skills/`, never deleted); a merge of two HPC-built skills is still proposed. Done now: `weekly-check` (collided with `month-plan` on "how are we tracking", read Stripe) and `write-an-ad` (collided with `meta-ad-build` on "Meta ad", no HPC rules) retired, also approved in the OS. Approvals: recommendation is one plain sentence, change up to three short lines, evidence under a collapsed "Why · evidence"; `approvals.js` gained `edit` and `decide`; limits 200 / 500 characters. Updated: improve instructions, the weekly-improve task, maya.md, how-this-brain-works, setup, support, automation-guide.

**`meta-ad-build` skill built** · Evan, 2026-09-25 (yes to the wrap proposal). Paid image ads: build sheet from approved copy + library creative, sizes by the ad set's real placements, duplicate in Ads Manager, Evan publishes, ROUX reads back. New step: before Publish, "Review and publish" must list only the planned changes. → [skill](../my-skills/meta-ad-build/instructions.md)

**Why `18qt-001` got a new creative on 2026-09-25: an unpublished edit on it went out with the Tailgate A + B publish** · ROUX, read-only, 2026-09-25. Meta activity log: at 7:54 AM PT (Evan's account) the same publish created Lineup + Payoff, paused `004`, and swapped `001` from creative `1948549262494559` to `2115243779366697`; `001` went Pending → In review → Active at 7:57. Both creatives read identical (same body, headline, Shop Now, image hash `6f5046…`); only the FB/IG post changed (`…1997930737889936` → `…2012386576444352`), so `001`'s old likes and comments no longer show. **Where the edit came from is untested** (the duplicate step, or an older opened Edit panel; the old creative itself is dated 2026-09-11, the day after the known edit-panel draft). Nothing to undo. Watch `001` in Finn's Mon 9/28 read for a delivery dip. Prevention now in `meta-ad-build` step 4.

**`HPC10` still can't reach the kits (read-only re-check)** · ROUX, 2026-09-25, Shopify GraphQL. `HPC10`: ACTIVE, "10% off All Products - Eligible for Discounts • One use per customer", last edited 2026-09-15 (before the Sept 21 admin check that the collection excludes `Bundle`). All three kits still carry `Bundle` + `Sale-NoDiscount`. The connector still can't see that collection, so the admin check (Sept 21, and Evan's Sept 25) remains the proof. Evan + Jay's order test covers the rest; no checkout test needed.

**`rolling-boil` kill rule built** · ROUX in Ads Manager on Evan's "you build it", 2026-09-25. Rule `rolling-boil kill line $583.74 lifetime`: 1 ad (`52508008680391`), turn off ad if Spent > $583.74, time range Maximum (lifetime), runs continuously, Facebook notification. Read back: "Active automated rules" lists it for that ad. No older $389 rule existed on the ad (checked before building). Finn's Mon 9/28 read still covers tagged orders; a tagged order first → Evan turns the rule off.

**Tailgate A + B live in `18qt_prospecting` as feed + search ads; no Stories/Reels** · Evan, 2026-09-25 (option A). `18qt_prospecting` `6998161994987` runs manual placements with no Stories or Reels (connector read + Ads Manager "excluded for this ad set"), so the 9:16 renders cannot serve there. Both ads (`18qt-tailgate-kit-lineup` `52511818941191`, `-payoff` `52511820288791`): 1:1 → Feeds only, 1200×628 → Search results only, 9:16 removed. ROUX (Evan's yes, Chrome, draft only) also removed Meta AI image + video variations from Lineup, swapped Meta's AI-extended crops of the wide image for plain padding, and turned off Enhance CTA, brightness/contrast and product browsing on both (Relevant comments left on). Evan publishes. `18qt-004` paused same day (Evan). **Open for Beau:** whether to add Stories/Reels to the ad set (affects `001`–`003`, may reset learning). In Meta's current editor, per-image placement lives in Customize media → Placements, not Aspect ratios.

**Tailgate kit live; orders carry component SKUs; Leg Extensions stock count is wrong, not short** · Evan, 2026-09-25. Evan set the kit Active + Online Store. Order #17527 (draft order, 08:54 CT, now $0/closed) was Evan and Jay's test. Shopify read: its line items are the four component SKUs (`PWFRBR-VLV025B` $318.98 · `LegExtensions` $111.64 · `HP-5in-Therm` $15.00 · `SC-7R` $9.38 = $455), the bundle SKU only as the `lineItemGroup` label, fulfillment by component. Storefront: page live, all 4 variants at list price, add-to-cart OK (ROUX, browser); checkout/`HPC10` step not run (auto-mode block). **`LegExtensions` reads −17 in Shopify but legs are in, "a ton of them"** (Evan): the count is wrong, not a shortage; don't raise it as a stock risk. Fixing the count is Evan's or Jay's click.
...
**Affiliate program stays as is; no keep/remove list; QuickBooks fix is Jay's with Rhonda** · Jay and Evan, 2026-09-24 ("it's making money so it is okay"). Deal sites and no-website accounts keep earning commission; Beau's keep/remove list is dropped. The QuickBooks reclassification is Jay's to take up with Rhonda, so ROUX sends nothing and tracks nothing on it. The UpPromote Jay text is done.

**Review feedback routed; don't re-flag these from future review pulls** · Evan, 2026-09-24. Brad's missing $25 off (store review, 2026-07-31): **handled by Biljana**. Bigger fish-fryer drain spout: **no**, long-standing ask, but the bigger valves are too big for the pot. Shorter leg extensions: **with Jay**, deciding whether they're worth selling. Handle on the 20" skimmer: **yes in principle, parked**, no supplier yet. Source: Scout's customer-language file, 2026-09 → `my-files (knowledge)/hpc-reference/customer-language/2026-09-customer-language.md`.

**UpPromote Jay questions answered: the $11.7k is real and paid, the $4k line is a QuickBooks misclassification** · Jay, in person with Evan, 2026-09-24. **Facts (Jay):** payouts go out by PayPal; the gap is expenses booked to the wrong QuickBooks account, and Rhonda (accounting, rhonda@highperformancecookers.com) reclassifies them so QuickBooks matches UpPromote's Paid total. Jay cut the rate 8% → 5% himself (other sources said 5% was enough; protect margin); affiliates were told and see the rate in their contract when they log in. Payouts run every 21 days above a minimum (Jay thinks $15, not certain); small balances also wait on affiliates who never set up PayPal. No side deals with any affiliate or site. Google-Ads-flagged orders: a customer can click our ad and an affiliate link and buy through the link; nothing to do about it, so commission keeps being paid on them. **Open:** the Q3 buckets were unclear to them (explained 2026-09-24; keep/remove call still Jay's). **Check against the export (ROUX):** 19 of 21 unpaid affiliates do have a PayPal email on file; every unpaid balance over $15 is from Sept 8 or later, which fits the 21-day cycle and minimum. Corrected the draft's "owed $203.11" and "paid every one" before Jay saw it. → [Jay text](../my-work%20(outputs)/internal/reports/2026-09-21-uppromote-settings-and-jay-text-beau.md)

**30 QT CAC ceiling set: $105.06 per new customer ($95.58–$105.06), kill at $315.18 (3×) / $210.12 (2×)** · Beau's call, 2026-09-24, approved via Evan's proposal yes (`p-2026-09-24-30qt-ceiling`). **Method:** the incremental rule, same as 120 QT Powered $195.34 / Performance $189.23: break-even (net − landed COGS − 3.9% × net − $3.30) − 20% × net, no overhead charge. **Facts:** Jun–Aug 30 QT order mean $498.41 net · $270.93 landed COGS, 19 orders (Finn, Pull A, 2026-09-10 order-level-pulls); variant prices and `unitCost` match what-we-sell.md and Shopify (connector, 2026-09-24); only the $462.50 kit is in stock (61; $395 at 0, $442.50 at −2). $95.58 = one $462.50 kit, the figure to scale on; gate on Meta's count $38.23. 12-month basis not used (deleted-variant costing). Cross-check: the one-bird Turkey Fry Kit plan's $104.47. **The two live ads:** `Video_Jay 30qt (turkey) Fryer Demo` `6810865162787` lands on the 30 QT Turkey Fryer page and is taggable (102 sessions under its ID, Sept 1–24), $133.02 · 6 Meta · 0 tagged new → 3× line $315.18, ok. `IMG_Heavy Duty Heat.30qt` `6845666149587` has no sessions under its own ID, so it is read on Meta's count alone; 2 Meta purchases, so it can't fire. **Why:** the Thursday kill-line read had both 30 QT ads as "not read" for want of a ceiling. Caveat: 19 orders is thin; rebuild on a Sept–Nov pull. Meta shows $22 per purchase on the Jay 30 QT ad and Shopify shows 0 — don't scale on Meta's number. Added to PLAN.md's ceiling list and campaigns.md (BPM). Nothing written to Meta or Shopify. → [Beau](../my-work%20(outputs)/internal/reports/2026-09-24-30qt-cac-ceiling-beau.md)

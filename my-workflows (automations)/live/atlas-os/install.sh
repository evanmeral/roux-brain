...
# 2. the launchd agents: the server (always on) and the 6:30 pulse
...
echo "6:30 pulse registered: $(launchctl print "gui/$(id -u)/com.atlas.pulse" | grep -c 'com.atlas.pulse') job"
sleep 2
...
if curl -s -m 3 http://localhost:4242/health >/dev/null; then
  ...
  echo "Atlas OS is registered but not answering yet."
  echo "macOS must allow 'Atlas OS' to read your Desktop folder (the vault lives there). If it asked, click Allow."
  echo "If it did not ask: System Settings > Privacy & Security > Files and Folders > Atlas OS > turn on Desktop Folder."
  echo "It retries on its own every 15 seconds; details in logs/launcher.log."
fi

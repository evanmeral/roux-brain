...
function postCard(p) {
  ...
  return `<article class="post-card is-${esc(badge)}" data-piece="${esc(p.id)}">
    ...
    <div class="post-main">
      ...
      ${(p.alternates || []).map((alt) => `<div class="post-alt"><span class="mono stamp">Alternate</span><div class="post-alt-l">${esc(alt.label)}</div><div class="post-alt-m">${alt.media.map((am) => am.exists ? `<a href="${esc(am.url)}" target="_blank" rel="noopener" title="${esc(am.file)}"><img src="${esc(am.url)}" alt="${esc(alt.label)}" loading="lazy"></a>` : `<div class="post-missing">missing file<br><span>${esc(am.file)}</span></div>`).join('')}</div></div>`).join('')}

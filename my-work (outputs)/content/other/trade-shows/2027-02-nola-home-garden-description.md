# Exhibitor Directory Description — New Orleans Home & Garden Show

**Show:** New Orleans Home & Garden Show, Feb 19–21 2027
**Booth:** 10×10 corner
**Field:** public exhibitor page / directory blurb
**Written:** 2026-09-08 · Pete
**Status:** draft — Evan to approve before it goes on the form

Audience: New Orleans-area homeowners skimming a directory in February, front of
crawfish season. Mostly residential backyard cooks. Most have not heard of us. The
blurb has two jobs — say what we make, and give a reason to walk to the booth.

Three lengths of the same description. Pick whichever fits the form's character cap.

---

## 1. Short — 41 words / **234 characters**

High Performance Cookers builds outdoor cookers, boil pots and burners in Covington, about 45 minutes up the road. Our patented hand-welded Tunnel Tubes put a pot at a rolling boil in as fast as about 7 minutes. Come time it yourself.

---

## 2. Medium — 81 words / **447 characters** *(most likely fit)*

High Performance Cookers builds outdoor cookers, boil pots and burners in Covington, about 45 minutes up the road. The speed is not in the burner. It is in our patented Tunnel Tubes, hand-welded across the bottom of every pot so the heat spreads out instead of hammering one spot. Rolling boil in as fast as about 7 minutes. About 90-second recovery when the next sack goes in. Up to 75% less propane. Come stand at the booth and time it yourself.

---

## 3. Long — 146 words / **798 characters** *(with paragraph breaks; 794 if pasted as one block)*

High Performance Cookers builds outdoor cookers, boil pots, burners and fish fryers in Covington, about 45 minutes up the road. Made in USA by a team of about twelve who weld the pots, pack the freight and answer the phone themselves.

The speed is not in the burner. It is in the Tunnel Tubes, tubes we hand-weld across the bottom of every pot so the burner's heat spreads over the whole bottom instead of hammering one spot. Patent No. 11,844,459. That gets you a rolling boil in as fast as about 7 minutes, about 90-second recovery when the next sack goes in, and up to 75% less propane. The pots are 4mm aluminum, built to last generations with good care.

Crawfish season is on. Come by the booth, watch a pot come up, and time it yourself.

Feel the heat. See the speed. Taste the difference.

---

## Claim check

- Tunnel Tubes described as tubes hand-welded to the **bottom of the pot**; speed attributed to the pot, not the burner. ✅
- Every number qualified — "as fast as," "about," "up to." ✅
- "Rolling boil" throughout. No "hard boil." ✅
- No competitor named. ✅
- No warranty stated — a blurb this short cannot carry both required qualifiers (residential, 120 QT or smaller), so stating it at all would be a false claim. Deliberately omitted. ✅
- Fryers named as **fish fryers** and never tied to crawfish; the crawfish line refers to a pot. ✅
- Leads with what we build and where, not with price or a discount. ✅
- Patent No. 11,844,459 (issued). If a logo is uploaded, use the **shield** — the circle logo still reads "patent pending." ✅

---

## Before Evan submits

**1. Confirm the character cap against the actual form.** Exhibitor directory forms
almost always cap the description and rarely publish the limit in advance. Open the
form, check the field's counter or `maxlength`, then pick the version that fits.
Counts above include spaces and punctuation. If the cap is tight, the Short version
still carries the three things that matter — what we make, where we are, the 7 minutes.

**2. Fields these forms usually ask for alongside the description.** Worth gathering in
one pass so this is a single sitting:

| Field | What we'd give |
|---|---|
| Company name (as printed) | High Performance Cookers |
| Description | one of the three above |
| Logo file | high-res PNG **and** vector/EPS, on transparent and white. **Shield logo, not the circle.** Ask what dimensions and file size they cap at. |
| Product / exhibitor categories | usually a pick-list — outdoor living, grills & outdoor kitchens, backyard/patio, Louisiana-made. Pick every one that honestly fits; it drives directory filtering. |
| Website | highperformancecookers.com |
| Social handles | IG @highperformancecookers · FB /highperformancecookers |
| Booth contact | name, cell, and email for the person actually standing at the booth on show days — not necessarily the person signing the contract |
| Billing / admin contact | for invoices and move-in paperwork |
| Address | 17991 Painters Row, Ste 100, Covington, LA 70435 |
| Phone for the public listing | 985-260-1505 |
| Booth number | assigned by the show — may be blank until they confirm the corner |
| Show special / attendee offer | many directories have this field. **Decide deliberately — do not fill it in reflexively.** We lead with quality, not discounts, and a directory-wide discount is a pricing decision. Route to ROUX before anything goes in this box. |
| Photos for the listing | 2–3 lifestyle shots. Ask Evan which, and in what treatment. |
| Certificate of insurance / booth staff list | usually a separate deadline from the directory. Note it when the form appears. |

**3. Also unconfirmed:** whether the directory deadline is earlier than the booth
payment deadline. Directory copy usually closes weeks before the show so the printed
program can go to press. Check both dates when the exhibitor kit arrives.

---

**Next:** Ada — once Evan picks a version, this needs the exhibitor form filled and the
directory deadline on the calendar.
